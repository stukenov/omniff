...
MODELS = [
    ...
    ModelRecommendation(
        "Qwen2.5-VL-3B", "Qwen/Qwen2.5-VL-3B-Instruct", 7.0, "good", ["image", "video"]
    ),
    ModelRecommendation(
        "Qwen2.5-VL-7B", "Qwen/Qwen2.5-VL-7B-Instruct", 15.0, "excellent", ["image", "video"]
    ),
    ...
    ModelRecommendation(
        "SDXL-turbo", "stabilityai/sdxl-turbo", 5.0, "good", ["image_gen", "image_edit"]
    ),
...
def recommend_models(
    ...
    candidates = [m for m in MODELS if any(p in m.pipelines for p in pipelines)]
