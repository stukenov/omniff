...
from omniff.reliability import ModelMutex, VRAM_ESTIMATES, get_free_vram_gb, TimeoutError
...
class OmniFFRuntime:
    ...
    def _ensure_model(self, name: str, model_cls: type, trace: RequestTrace | None = None, **kwargs: Any) -> Any:
    ...
    def run(
        ...
        dispatch = {
            "TEXT_SIMPLE": lambda: self._run_text_to_text(prompt or input, route.route_class, thinking, controls, trace),
            "TEXT_NORMAL": lambda: self._run_text_to_text(prompt or input, route.route_class, thinking, controls, trace),
            "TEXT_COMPLEX": lambda: self._run_text_to_text(prompt or input, route.route_class, thinking, controls, trace),
            "IMAGE_CAPTION": lambda: self._run_image_to_text(input, prompt, controls, trace),
            "AUDIO_TRANSCRIBE_ONLY": lambda: self._run_audio_to_text(input, prompt, controls, trace),
            "AUDIO_QA": lambda: self._run_audio_to_text(input, prompt, controls, trace),
            "IMAGE_EDIT": lambda: self._run_image_to_image(input, prompt or "", controls, output, trace),
        ...
        if handler:
            ...
            result = RunResult(output_text=f"Unsupported route: {route.route_class}", route=route.route_class)
    ...
    def _run_image_to_text(self, image_path: str, prompt: str | None, controls: dict, trace: RequestTrace) -> RunResult:
    ...
    def _fallback_image_metadata(self, image_path: str) -> RunResult:
        ...
        try:
            ...
            pass
        return RunResult(
            output_text=info, route="IMAGE_CAPTION",
            metadata={"fallback": True},
        )

    def _run_audio_to_text(self, audio_path: str, prompt: str | None, controls: dict, trace: RequestTrace) -> RunResult:
    ...
    def _run_image_to_image(self, image_path: str, prompt: str, controls: dict, output: str | None, trace: RequestTrace) -> RunResult:
        from omniff.models.image_edit import ImageEditModel

        model_id = controls.get("model_id", "stabilityai/sdxl-turbo")
        editor = self._ensure_model("image_edit", ImageEditModel, trace=trace, model_id=model_id, device="auto")
    ...
    def _run_text_to_image(self, prompt: str, controls: dict, output: str | None, trace: RequestTrace) -> RunResult:
        from omniff.models.text_to_image import TextToImageModel

        model_id = controls.get("model_id", "stabilityai/sdxl-turbo")
        gen = self._ensure_model("text_to_image", TextToImageModel, trace=trace, model_id=model_id, device="auto")
        output_path = output or "output.png"
    ...
    def _run_video_to_text(self, video_path: str, prompt: str | None, controls: dict, trace: RequestTrace) -> RunResult:
        from omniff.models.video_captioner import VideoCaptionerModel

        model_id = controls.get("model_id", "Qwen/Qwen2.5-VL-3B-Instruct")
        captioner = self._ensure_model("video_captioner", VideoCaptionerModel, trace=trace, model_id=model_id, device="auto")
    ...
    def _run_document_to_text(self, doc_path: str, prompt: str | None, controls: dict, trace: RequestTrace) -> RunResult:
